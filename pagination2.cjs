...
// Find the exact block in the table body. Since there's multiple instances, we'll replace the one after 'Historia de ventas'
...
const tableEnd = code.indexOf('</table>', tbodyStart);
const tableDivEnd = code.indexOf('</div>', tableEnd);
if (tableDivEnd !== -1) {
   code = code.substring(0, tableDivEnd + 6) + paginationControls + code.substring(tableDivEnd + 6);
}
